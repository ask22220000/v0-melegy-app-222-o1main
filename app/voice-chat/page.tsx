...
// ─────────────────────────────────────────────────────────────────────────────
// GIF-accurate Orb — dark glass sphere with teal/purple haze + dot mesh
// ─────────────────────────────────────────────────────────────────────────────
function OrbCanvas({
  orbStateRef,
  analyserRef,
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillStyle = sphG; ctx.fill()
      ctx.restore()

      // 3 – clip everything inside sphere
      ctx.save()
      ctx.beginPath(); ctx.arc(cx, cy, R * 0.996, 0, Math.PI * 2)
      ctx.clip()
      ...
      // 3d – central pill glow (the bright IK-zone in GIF)
      const coreAnim =
      ...
      ctx.beginPath(); ctx.arc(0, 0, cW, 0, Math.PI * 2)
      ...
      const glG = ctx.createRadialGradient(cx - R*0.40, cy - R*0.44, 0, cx - R*0.18, cy - R*0.22, R*0.54)
...
export default function VoiceChatPage() {
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      // mic → analyser (no output: avoids echo)
  ...
  const handleRecordingStop = useCallback(async () => {
    ...
    try {
      ...
      setError(e.message); setOrbState("idle"); return
    }
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok || !data.reply) throw new Error(data.error || "فشل الحصول على رد")
    ...
    try {
      ...
      audio.addEventListener("canplay", () => {
        try {
          ...
          const analyser = actx.createAnalyser()
          analyser.fftSize = 256
          // GainNode to boost volume
          const gain = actx.createGain()
          gain.gain.value = 2.0          // 2× amplification
          ...
          // play without analyser if AudioContext fails
        }
      }, { once: true })
    ...
    >
      ...
      <div className="w-full flex items-center justify-between px-6 py-5">
        <button
          ...
          className="flex items-center gap-2 text-white/40 hover:text-white/75 transition-colors text-sm"
        >
          <ArrowRight className="h-5 w-5" />
          <span>رجوع</span>
        </button>
        <span className="text-white/20 text-xs tracking-widest uppercase">MELEGY VOICE</span>
        <div className="w-16" />
      </div>
      ...
      <div className="flex flex-col items-center flex-1 justify-center gap-4">
        ...
        {/* State label */}
        <p className="text-white/55 text-base font-medium tracking-wide mt-1">
          {stateLabel}
        </p>

        {/* Transcript */}
        {transcript && (
          <div className="max-w-sm text-center px-6" dir="rtl">
            <span className="block text-white/22 text-xs mb-1">قلت:</span>
        ...
        {reply && (
          <div className="max-w-sm text-center px-6" dir="rtl">
            <span className="block text-cyan-400/30 text-xs mb-1">ميليجي:</span>
            <span className="text-cyan-300/70 text-sm leading-relaxed">{reply}</span>
          </div>
        )}

        {/* Error */}
        {error && (
          <p className="text-red-400/75 text-sm px-6 text-center" dir="rtl">
            {error}
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col items-center gap-4 pb-14">
        {orbState === "idle" && (
          <button
            ...
            className="flex items-center gap-3 px-8 py-4 rounded-full
              bg-cyan-500/12 hover:bg-cyan-500/22 border border-cyan-500/30 hover:border-cyan-400/50
              text-cyan-300/85 text-sm font-bold transition-all duration-200 active:scale-95"
          >
        ...
        {isRecording && (
          <button
            ...
            className="flex items-center gap-3 px-8 py-4 rounded-full
              bg-red-500/12 hover:bg-red-500/24 border border-red-500/30 hover:border-red-400/55
              text-red-300/85 text-sm font-bold transition-all duration-200 active:scale-95"
          >
        ...
        {(orbState === "thinking" || orbState === "speaking") && (
          <p className="text-white/18 text-xs tracking-wide">
            {orbState === "thinking" ? "جاري المعالجة..." : "يرجى الانتظار..."}
          </p>
        )}
      </div>
    </main>
